...
After creating the schedule, the response will include a list of milestones with their IDs. For each screening step the user confirmed as already completed (TB screening, hepatitis screening, baseline labs, prior authorization, iPLEDGE registration, etc.), immediately call medication_schedule with action="complete", milestone_id=<the numeric ID from the create response>, and completed_date=<the date the user provided>. You MUST extract the milestone IDs from the create response milestones array — each milestone has an "id" field. For example, if the create response shows a milestone with id=801 and step_name="tb_screening", and the user said TB was done on 2/28/26, call: medication_schedule(action="complete", milestone_id=801, completed_date="2026-02-28"). Do this for EVERY milestone the user confirmed was done.
